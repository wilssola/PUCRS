/**
 * Demonstração automática do sistema de controle de estacionamento da EstACME.
 * Programação Orientada a Objetos - PUCRS - Wilson Oliveira Lima
 *
 * Execute com: node src/demo.js   (ou npm run demo)
 *
 * Diferente de src/main.js (interface interativa), este arquivo executa um
 * roteiro fixo que exercita todas as regras de negócio, sem pedir dados ao
 * usuário. Trabalha na pasta data-demo/, para não alterar os dados reais.
 *
...
const PASTA_DADOS = path.join(PASTA_PROJETO, 'data-demo');
...
// restaurarCenario: parte sempre do cenário original, então a demonstração
// pode ser executada quantas vezes forem necessárias
app.iniciar({ restaurarCenario: true });
...
app.tentar('Estudante cadastrado (limite de 1 placa)', () => app.cadastro.cadastrarCliente(estudante));
app.tentar('Segunda placa do estudante deve ser recusada', () => app.cadastro.adicionarPlaca('11122233344', 'IEE2I22'));

const professor = new Professor('22233344455', 'Carlos Pereira');
professor.adicionarPlaca('PRO1P11');
professor.adicionarPlaca('PRO2P22');
app.tentar('Professor cadastrado (limite de 2 placas)', () => app.cadastro.cadastrarCliente(professor));
app.tentar('Terceira placa do professor deve ser recusada', () => app.cadastro.adicionarPlaca('22233344455', 'PRO3P33'));

const empresa = new Empresa('11222333000144', 'Alfa Serviços Ltda.');
['EMP1E11', 'EMP2E22', 'EMP3E33'].forEach((p) => empresa.adicionarPlaca(p));
app.tentar('Empresa cadastrada (sem limite de placas)', () => app.cadastro.cadastrarCliente(empresa));
app.tentar('Placa já usada por outro cliente deve ser recusada', () => app.cadastro.adicionarPlaca('11222333000144', 'IEE1I11'));
